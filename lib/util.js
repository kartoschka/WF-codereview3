  ...
  static make_domobj(...args) {
    ...
    function _make_domobj(a) {
      if(a instanceof Element) {
        ...
        if(classes.length > 0) elmt.classList.add(classes)
...
// toying around with data flow ideas or something
class Channel {
  constructor(input, output) {
    this.input = input
    this.output = output

    if(this.input.send) throw "class connected with Channel must not implement .send()"

    this.input.send = (...data) => this.transmit(data)
    if(!this.output.receive) this.output.receive = (data) => console.log("received message: " + data)
  }

  transmit(data) {
    this.output.receive(...data)
  }
}
