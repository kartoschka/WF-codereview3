...
class LibraryItem {
  ...
  toggleLike() {
    if(this.likedByUser) {
      this.like_count_change_effect("lightpink")
      this._data.userLikeState = 0
      this._like_button.className = "rating-inactive"
    } else {
      this.like_count_change_effect("lightblue")
  ...
  like_count_change_effect(color) {
    this._like_count_elmt.style.transition = null
    this._like_count_elmt.style.backgroundColor = color
    setTimeout(()=>{
      this._like_count_elmt.style.transition = "background-color 2s ease"
      this._like_count_elmt.style.backgroundColor = "transparent"
    }, 1000)
  }
